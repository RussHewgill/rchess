...
fn main_threading() {
    ...
    let params = "e2e4 c7c5 g1f3 d7d6 d2d4 c5d4 f3d4 g8f6 b1c3 a7a6 c1g5 e7e6 g5f6 d8f6 g2g3 b8c6 d4c6 b7c6 f2f4 a8b8 d1c1 d6d5 e4e5 f6e7 a2a3 c6c5 f1g2 c5c4 c3d5 e6d5 g2d5 c8b7 d5b7 e7b7 e1d2 b7d5 d2e3 f8c5 e3e2 d5g2 e2e1";
    let params = params.split(" ");
    let moves: Vec<&str> = params.clone().collect();

    let mut moves2 = moves.clone();
    moves2.pop();
    moves2.pop();

    ex.update_game_movelist(&ts, STARTPOS, moves2.into_iter());
    let g = ex.game;
    eprintln!("g = {:?}", g);
    let (mmove, stats) = ex.explore();
    eprintln!("mmove = {:?}", mmove);
