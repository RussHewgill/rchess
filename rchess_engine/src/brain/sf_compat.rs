...
const PIECE_SQ_INDEX: [[usize; 16]; 2] = [
...
fn orient(king_sq: u8, side: Color, sq: u8) -> u8 {
    let p = side.fold(0, 1);
    const A8: u64 = 56;
    const H1: u64 = 7;
...
pub fn make_index_half_ka_v2(king_sq: u8, side: Color, sq: u8) -> usize {
    let o_king_sq = orient(king_sq, side, sq);

    // const WID: [usize; 8] = [0, ]

    // let pidx = if side == White {
    // } else {
    // };
